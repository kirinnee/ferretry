...
| `:term/<id>`       | A terminal instance | `:term/0a1b2c3d4e5f`          | Twelve lowercase hex digits, as the daemon mints them       |
| `:page/<id>`       | A browser page      | `:page/PAGE-1`                | The worker's own page id                                    |
...
`:term/` and `:page/` share the agent sigil deliberately: `:` means "a live thing in this session you
can open". A callsign can never contain `/`, so an agent literally named `term` still resolves as
`:term`.
...
| `:term/<id>` | That exact terminal instance's tab                                       |
| `:page/<id>` | Nothing yet — see section 8                                              |
...
libraries are not an `@` tier either.

Three Settings switches govern suggestions only — every repeated-`@` tier, the `&`/`:`/`!` direct
references, and `$` skills. All default on. **A disabled switch suppresses the menu, never the
...
> Not yet true in this build: the direct `:`/`&`/`!`/`$` triggers, the Settings switches, and the
> removal of the `@@@@@` Pins tier are handover #33 and #63. The token grammar in this document is
> settled and implemented; those are the menus around it.
...
| `terminal`  | The instance is registered in this session                     |
| `browser`   | The page is registered in this session                         |

A surface with no resolver for a kind cannot prove it, so that kind stays prose there. A missing
...
`browser` is in exactly that intermediate state today: it parses, proves and encodes, and it has no
opener, because a browser tab is addressed by its destination and a page id is not one. Handover #64
gives a registered page a resolvable identity and closes it.
