...
| Remotely use terminal, browser, and file system                         | CLI has local attach/read commands plus `fy fs ls/cat/changes/diff` and browser command verbs.                                                                                                   | Filesystem reads, terminal lifecycle/socket routes and terminal-scoped one-use ticket minting are mounted. Per-session browser GET/POST intentionally returns 501: the worker and transport exist, but no per-session runtime composes them.                         | `FilesTab` safely previews rich files. `SessionTerminalDeck` mounts an `@xterm/xterm` co-control surface with daemon-bound one-use tickets; the agent pane snapshot remains read-only. File requests can relay, but the terminal stream is direct-only.                                                                                             | **Partial.** Remote file browsing/previewing and interactive terminal co-control work; browser automation does not yet work end to end.                                                                                     | Relay carries request/response only, not the terminal WebSocket, so a remote terminal needs direct daemon reach. Browser panes are merged but unproven, and the session host still lacks the composition from session id to launched worker and production `BrowserViewerHost`; its route remains an honest 501.                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Configure my fleet from the UI                                          | `fy fleet init` and `fy fleet apply` operate locally. `fy fleet authorize <proposal-id>` mints one single-use host approval for one exact reviewed change and refuses `--json`.                  | `POST /v1/fleet/proposals` holds a derived, revision-bound `initialize`, `create-account` or `edit-account` proposal; permissions, asset reads, host-only authorization and application are mounted.                                                                 | The Fleet sub-tab prepares a host or creates/edits an account layer, previews the numbered change manifest, shows the host approval command, accepts its code and renders each parsed outcome. Switching daemon drops draft, proposal, code and result.                                                                                             | **Yes, with the authority condition stated before the first click.** A host admin applies directly; a paired device composes and reviews, then applies with one host-minted approval for that exact change.                 | No account removal or harness/wrapper/home edits. `profiles`, `variants`, `commands`, `aliases`, `defaultHomes`, `sharedHistory`, `health` and `usage` remain hand edits of `config.yaml`; per-profile environment is read-only in the browser. Applying rewrites parsed YAML, inline settings merge but cannot delete a key, and only text assets are editable.                                                                                                                                                                                                                                                                                                                                          |
...
- The workspace now mounts `SessionTerminalDeck` as the real, interactive
  ...
  read-only. The terminal WebSocket is direct-only. The browser runtime remains
  an explicit 501 despite the merged pane components.
...
- PR #276 mounted the Fleet Settings sub-tab and its host-authorized proposal
  ...
  backs proposal routes in `runtime/mounts/fleet.ts`; `fy fleet authorize` is
  the host half, and `packages/pwa/src/features/fleet/` is mounted through
  `daemonSettingsTabs` in `App.tsx`.
...
3. **Decide the terminal's relay reach.** The PWA `@xterm/xterm` deck, ticket
   minting and socket bridge are mounted; the cached session-pane snapshot is
   deliberately a separate read-only fallback. Relay transports request/response
   routes only, so a terminal stream currently needs direct daemon reach.
