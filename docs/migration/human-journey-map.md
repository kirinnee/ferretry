...
row was re-audited on 2026-08-05 against the browser-dictation change
(`64b626f1`…`6bc5dcbb`); every other row still carries its 2026-08-03 reading.
...
- PRs #126 and #134 described a daemon-bound dictation engine. That engine is
  gone: `64b626f1` moved recognition into the browser's SpeechRecognition API
  and `638a935e` deleted the record-then-upload pipeline (`audio-capture.ts`,
  `pcm.ts`, `silence-segmenter.ts`, `utterance.ts`, `daemon-engine.ts`,
  `capabilities.ts`, `capture-error.ts`, `ort-precache.ts`); `6bc5dcbb` removed
  the PCM16 worklet that fed it. Browser dictation is the shipped path now. A
