...
| Remotely use terminal, browser, and file system                         | CLI has local attach/read commands plus `fy fs ls/cat/changes/diff` and browser command verbs.                                                                                                                                                    | Filesystem read routes and terminal lifecycle/socket routes are mounted; a paired device can mint a single-use ticket bound to one terminal stream. Attach is explicitly loopback-only. Browser login is mounted, but per-session browser GET/POST returns 501. | Files, terminal snapshot, terminal stream, remote-browser and unified-browser components exist; the PWA can mint the terminal ticket and build its ticket-only URL, but no production session workspace mounts them. | **No.** Filesystem reads are CLI-only; none of the requested remote workbench is usable from the PWA.                                          | The terminal transport is now reachable once a session workspace hosts its pane. Browser automation additionally lacks its browser worker and viewer host, so `/v1/sessions/:id/browser` is an intentional 501. A workspace host is the first common missing link for files and terminal UI.                                                                                   |
...
| Cost tracking                                                           | `fy analytics` queries spend, tokens, duration and outcomes; `fy fleet usage` reports account quota.                                                                                                                                              | Analytics is mounted and indexes session evidence. Each daemon reads its own operator-owned `analyticsPricing` catalog from `config/daemon.json`.                                    | The routed Global Analytics page has query, table and time-series UI; session dashboards show quota.                                                                                             | **Partial.** Operator-supplied prices can value known usage; subscription spend remains a separate measurement.                                | An omitted price stays unpriced, and malformed or ambiguous catalog input refuses daemon startup rather than producing a benign zero. The PWA continues to label values as equivalent/public-API cost, not subscription spend.                                                                                                                                                 |
