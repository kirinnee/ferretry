...
**Merge gate, all three, every time:** `pre-commit run --all-files`, `task test`, and the snapshot
publish. **Never weaken a gate to make something pass** — no blanket knip ignores, no `|| true`, no
`@ts-ignore`, no loosened tsconfig. This refactor has no original to diff against, so the gates and
the tests are its only oracle.
