...
| Settings for text size, density, chat width, composer Markdown, Enter-key behaviour, themes, dictation, notifications, PWA identity, and Warden link | PORTED device settings: `packages/pwa/src/features/settings/settings-page.tsx`; PORTED composer key capability from `ui/src/components/Composer.tsx`: `components/composer.tsx`, `lib/composer-keybinding.ts`, and `features/settings/composer-enter-key-settings.tsx`; PORTED daemon-owned frame and Warden tab: `features/settings/daemon-settings-frame.tsx`. |
