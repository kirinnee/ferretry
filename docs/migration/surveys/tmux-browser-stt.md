...
longer exists. Recognition moved into the browser's own Web Speech API (`64b626f1`), the PWA's
record-then-upload pipeline was deleted with it (`638a935e`, `6bc5dcbb`), the CLI kept only text
enhancement (`de0e3274`), and the daemon's recogniser, model store and worker supervisor were
deleted (`6d2b5e7d`). The rows are kept rather than dropped, because "we ported this, then deleted
...
| `stt-audio.ts`       |   219 | —                                                                                                                    | **REMOVED** (`6d2b5e7d`). PCM16LE/WAV decode under a duration budget existed only for `transcribe`. No audio crosses the daemon boundary now.                                                                                                                                                                                                                                         |
| `stt-model.ts`       |   667 | —                                                                                                                    | **REMOVED** (`6d2b5e7d`) — catalogue, `.stt-model.json` manifests, download + sha256 verify + extract, install plans, and the `/stt-models/` browser feed. `tar` was the installer's only spawned process and `fy doctor` never checked for it, so an unchecked host dependency disappears with it.                                                                                   |
| `stt-paths.ts`       |    30 | —                                                                                                                    | **REMOVED** (`6d2b5e7d`). Models directory, STT state directory and `workerLog`; nothing survives that needs a path.                                                                                                                                                                                                                                                                  |
...
| `stt-worker.ts`      |   229 | —                                                                                                                    | **REMOVED** (`6d2b5e7d`). It was never a packaged artifact: the daemon's `bin` map ships `fyd` and the browser worker, and the release compile step takes only the first `bin` entry.                                                                                                                                                                                                 |
...
(`de0e3274`), where `enhance` is the group's default command and takes already-transcribed TEXT.
