...
| `answer`      | —                                        | **GAP** — see B; still downstream of F, see the note under B |
| `rename`      | —                                        | **GAP**                                                      |
...
| `answer`                                                        | 3438             | **GAP**, re-confirmed by unit ANSWERSWEEP against the source. Two things are missing, not one: (a) nothing WRITES `pendingQuestion`, which needs `reconcileStructuredQuestionFrame` and the transcript event handlers — F; (b) the DRIVE, `TmuxController.answerQuestion`/`cancelQuestion` plus roughly 1,150 lines of pane matchers in `tmux-controller.ts` (189–1350), none of which is ported. Neither half is useful alone: a drive with nothing to answer refuses, and a written question with no drive cannot be answered |
...
| `subscribe`                                           | 5027             | **GAP** — no fleet event subscription; `lib/api/socket.ts` streams terminals only. `fy stream <id>` is an explicitly per-session, cancellable poller over the mounted durable replay and is not presented as this missing socket.                                                 |
...
| `src/index.ts` `.command('stream')`; `ApiClient.stream`                         | `packages/cli/src/lib/reads/{commands,controller,render}.ts`; cancellable `FyApiClient.events` reads                                                                                      | **PARTIAL / PORTED AS A POLLER**. The required per-session long-lived read is executable, cancellable, and emits idle proof. The legacy optional-id fleet WebSocket remains GAP. |
...
| `src/index.ts` `.command('attach')`                                             | —                                                                                                                                                                                         | **GAP**. Public `SessionView.config` carries no tmux address. Guessing from a session id could select a same-named pane owned by another daemon.                                 |
...
| `rankFailoverCandidates` (migrate candidates in the prompt)      | —                | **GAP** — the prompt supports the list and the sweep supplies none, so the prompt's own rule (never migrate outside the list) means no migration rather than a guessed one                 |
