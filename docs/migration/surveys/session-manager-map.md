...
| `snapshot`, `lastSnapshot`                                                                         | 4702, 4721 | PARTIAL — live `snapshot` is **PORTED** by `lib/session/reads/service.ts`, `runtime/mounts/session-reads.ts`, and `GET /v1/sessions/:id/snapshot?live=true`. Stored `lastSnapshot` remains **GAP**: this daemon writes no last-frame artifact and explicitly refuses `live=false`. |
| `logs`                                                                                             | 4997       | PARTIAL — a provenance-resolved normalized transcript tail is **PORTED** by `lib/session/reads/service.ts` and `GET /v1/sessions/:id/logs`. Per-turn logs remain **GAP** because Ferretry writes no turn-partitioned log; `?turn=` is explicitly refused.                          |
...
| ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/index.ts` `.command('snapshot')`; `ApiClient.snapshot`                     | `packages/cli/src/lib/reads/{commands,controller}.ts`; `packages/protocol/src/adapters/fy-api-client.ts`; mounted live snapshot route above | **PORTED — live only**. Stored last-frame reads remain the explicit GAP above.                                                                                                   |
| `src/index.ts` `.command('logs')`; `ApiClient.logs`                             | Same CLI/controller/client chain; mounted transcript route above                                                                            | **PARTIAL**. Whole provenance-backed tail is executable; `--turn` is refused because no per-turn artifact exists.                                                                |
