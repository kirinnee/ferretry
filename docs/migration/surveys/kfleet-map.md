...
2. **Configuration is accepted and silently ignored.** `sharedHistory`, `health.*` and almost all of
   ...
   state" failure applied to configuration — see [Deliverable 2](#what-this-unit-closed).
...
| `core/harness-probe.ts:80` `sanitizeHarnessEnv`, `:97` `prepareHarnessProbeEnv` | **PLANNED BY THIS UNIT** | See "What this unit is closing".                                                                                                                                                                   |
...
| ------------------------------------------ | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core/login.ts:331` `interactiveLogin`     | `packages/fleet/src/adapters/process-login.ts:18` `ProcessFleetLoginPort` | **PORTED but unmounted** — there is no `fy fleet login` command.                                                                                                                                                                                                                                                                  |
...
| `cli/login.ts:214` `createLoginCommand`    | **PLANNED BY THIS UNIT**                                                  | `--status`, `--sync-only`, `--no-probe` remain GAPs (they depend on the rows above).                                                                                                                                                                                                                                              |
| `cli/login.ts:28` `nonLoginStatus`         | **PLANNED BY THIS UNIT** (as the `not-required` outcome)                  |                                                                                                                                                                                                                                                                                                                                   |
...
## What this unit is closing

> **Status: the audit above is complete and verified. This section states intent.** Each entry moves
> to past tense in the same commit that lands it; anything still in future tense at the end of the
> unit was not built.

Three gaps, chosen for value per line and for staying inside this unit's files.
