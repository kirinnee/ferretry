/**
 * ACCESSIBLE, THROTTLED resize control for the desktop side pane. Ported from
 * kteam `ui/src/components/SidePaneResizeHandle.tsx`.
 *
...
/**
 * Preserve a readable chat column before honouring the pane's absolute max.
 * Desktop layout guarantees enough room for the 320px minimum (the mobile
 * breakpoint uses a bottom sheet instead).
 */
export const sidePaneWidthBounds = (workspaceWidth: number): SidePaneWidthBounds => {
...
const clampToBounds = (width: number, bounds: SidePaneWidthBounds): number =>
...
export const sidePaneWidthFromPointer = (
  startWidth: number,
  startClientX: number,
  clientX: number,
  bounds: SidePaneWidthBounds,
): number => clampToBounds(startWidth + startClientX - clientX, bounds);

export const sidePaneWidthFromKey = (
  key: string,
  currentWidth: number,
  bounds: SidePaneWidthBounds,
  largeStep = false,
...
const UNCONSTRAINED_WORKSPACE_WIDTH = SIDE_PANE_MAX_WIDTH + SIDE_PANE_MIN_CHAT_WIDTH + SIDE_PANE_WORKSPACE_GAP;

const elementGeometry = (handle: HTMLButtonElement): { width: number; bounds: SidePaneWidthBounds } => {
  ...
  const workspaceWidth = workspace?.getBoundingClientRect().width ?? UNCONSTRAINED_WORKSPACE_WIDTH;
  return { width, bounds: sidePaneWidthBounds(workspaceWidth) };
};

export interface SidePaneResizeHandleProps {
  readonly width: number;
  readonly onPreview: (width: number) => void;
  readonly onCommit: (width: number) => void;
  /**
   * Optional deterministic test/embed bound. The normal workspace observes its
   * actual width and keeps ARIA synchronized as that width changes.
   */
  readonly bounds?: SidePaneWidthBounds;
}
...
export function SidePaneResizeHandle({
  ...
  return (
    ...
    // aria-valuenow/min/max. `<hr>` — the element the a11y rule suggests for
    // this role — cannot take focus, take keyboard commands or capture a
    // pointer drag, so it cannot express a resize control at all.
    // biome-ignore lint/a11y/useSemanticElements: a window splitter must be focusable; hr cannot be.
    <button
      ref={handleRef}
      type="button"
      role="separator"
