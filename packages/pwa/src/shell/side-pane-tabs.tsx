/**
 * THE UNIFIED TAB CONTROLS for the session side pane. Ported from kteam
 * `ui/src/components/SidePaneTabs.tsx`; one model (`side-pane-tab-model.ts`)
 * feeds two presentations:
 *
 ...
 *                    exactly ONE ROW tall: overflow is horizontal only, and the
 *                    + button stays pinned at the end.
 ...
 *
 * The controls are deliberately presentational: the side pane owns the
 * (daemon, session) scope and calls the model; later surfaces register tabs in
 * the model and appear here without this file changing.
 */

import { Check, ChevronsUpDown, Plus, X } from 'lucide-react';
import { useEffect, useId, useRef, useState } from 'react';
import { BottomSheet } from './bottom-sheet.tsx';
import { sidePaneTabIcon } from './side-pane-tab-icons.ts';
import type {
...
/** WAI-ARIA tab keyboard policy. Focus movement is invoked only from the strip's
 *  own key handler, never when a surface opens through another trigger. */
...
export interface SidePaneTabsProps {
  ...
  /** The strip's OPEN tabs (definitions resolved, strip order) — singleton
   *  registrations and synthesized instance definitions alike. */
  readonly tabs: readonly SidePaneTabDefinition[];
  /** Every registered tab, strip order — the picker's catalogue. Instance tabs
   *  are never in it; their catalogue entries are the `instanceKind`
   *  definitions (Browser) that spawn them. */
  readonly all: readonly SidePaneTabDefinition[];
  /** The active tab id. May name a tab outside `tabs` when its definition
   *  vanished; the strip then simply has no selected tab. */
  readonly current: SidePaneTabId;
  /** Switch among open tabs. */
  readonly onSelect: (id: SidePaneTabId) => void;
  /** Add a not-yet-open tab to the strip and activate it (the + picker). */
  readonly onAdd: (id: SidePaneTabId) => void;
  /** Take a tab out of the strip; closes (disposes) an instance tab. */
  readonly onRemove: (id: SidePaneTabId) => void;
  ...
  readonly onNewInstance?: (kind: SidePaneInstanceKind) => void;
}

type PickerListProps = {
  readonly all: readonly SidePaneTabDefinition[];
  readonly openIds: readonly SidePaneTabId[];
  readonly onAdd: (id: SidePaneTabId) => void;
  readonly onRemove: (id: SidePaneTabId) => void;
  readonly onNewInstance?: (kind: SidePaneInstanceKind) => void;
};

/** Rows shared by the desktop + picker and the mobile modal's "Add" section: one
 *  toggle per registered tab, pressed = in the strip. An unavailable tab (mcp)
 *  stays togglable — it is a real default tab whose BODY is the honest
 *  placeholder; hiding the choice would misrender absence as nonexistence. An
 *  `instanceKind` entry is not a toggle at all: choosing it always creates a NEW
 *  instance tab, so it reads "New …", never "Remove …". */
export function SidePaneTabPickerList({ all, openIds, onAdd, onRemove, onNewInstance }: PickerListProps) {
  return (
    <ul className="m-0 flex list-none flex-col gap-xs p-0">
      {all.map(def => {
        const Icon = sidePaneTabIcon(def.icon);
 ...
 *  removal), then the not-yet-open catalogue under "Add a tab". Exported so the
 *  markup contract is testable without opening the sheet. Instance tabs list
 *  with their short label (full path/URL as the hover title) and the same
 *  per-row ✕ every tab gets — NEVER a horizontal strip. */
...
/** Desktop: the tablist of open tabs plus the sibling + button and its anchored
 *  non-modal picker (pointer-down-outside and Escape dismiss it, matching the
 *  launcher popover's manners). */
function SidePaneTabStrip({
  ...
  // Roving tabindex needs exactly one stop even when the active tab has no strip
  // entry (its definition vanished): fall back to the first tab.
  ...
  return (
    <div className="flex shrink-0 items-stretch border-b border-border-soft">
      ...
      >
        {tabs.map(tab => {
          const Icon = sidePaneTabIcon(tab.icon);
          ...
          return (
            <button
              ...
              title={tab.label}
              onClick={event => {
                // The ✕ lives INSIDE the tab (a nested button is invalid inside
                // role=tab), so the tab's own handler decides which of the two
                // affordances was hit. kteam put the handler on the ✕ itself; an
                // interaction handler on a presentational element is exactly what
                // the a11y lint forbids, and routing it here changes nothing a
                // reader can observe.
...
/** Phone: the single tab control. No tablist exists in this presentation — the
 *  control names the active tab and opens the switcher modal. */
...
export function SidePaneTabs({ presentation, ...rest }: SidePaneTabsProps) {
