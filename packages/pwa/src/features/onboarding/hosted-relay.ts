...
/**
 * The intended order, stated as the design and not as a measurement.
 *
 * "Tries" would be a claim about behaviour that does not exist yet, so this says
 * what the carriers ARE and `TRANSPORT_NOT_WIRED_NOTE` says what happens today.
 */
export const CARRIER_ORDER_NOTE =
  'There is nothing to choose here. A direct connection to your own daemon is what Ferretry uses whenever the ' +
  'browser can reach it; the hosted relay exists to carry the traffic when it cannot.';
