...
export function DoctorSettings({ connection, read }: { readonly connection: DaemonConnection; readonly read: DoctorReportReader }) {
  ...
  return (
    <section className="kt-panel p-panel" aria-labelledby="doctor-settings-heading" data-doctor-daemon={String(connection.daemonId)}>
      <div className="flex items-center gap-2">
        <Stethoscope size={17} className="text-accent" aria-hidden="true" />
        <h3 id="doctor-settings-heading" className="m-0 text-title font-semibold text-fg">Host checks</h3>
      </div>
      <p className="mb-3 mt-1 text-ui leading-base text-muted">
        {report.ready ? 'Required dependencies are present.' : 'A required dependency is missing; sessions will not work yet.'}
      ...
      <ul className="m-0 flex list-none flex-col gap-2 p-0" aria-label="Host dependency checks">
        {report.checks.map(check => {
          const Icon = check.status === 'present' ? CircleCheck : check.status === 'missing' ? CircleAlert : CircleMinus;
          ...
          return (
            <li key={check.name} className="rounded-control border border-border-soft bg-surface-2 px-3 py-2">
              ...
              <p className="mb-0 mt-1 text-meta leading-base text-muted">{check.summary}. {check.impact}</p>
            </li>
          );
        })}
      </ul>
