/**
 * The keyboard contract every overlay in this app owes the reader. Ported from
 * kteam `ui/src/hooks/useDialogFocus.ts`.
 *
 * Four things, none of them optional once a container claims `aria-modal="true"`:
 *
 ...
 *                than no dialog.
 *   TRAP       — Tab cycles within the dialog. `aria-modal` claims the rest of
 *                the page is unreachable; a claim the reader can disprove with
 *                one keystroke is worse than not making it.
 *
 ...
 *
 * ESCAPE IS A STACK, NOT A BROADCAST. Overlays here can genuinely be open at
 * the same time — the session details sheet with the command palette summoned
 * over it is the ordinary case — and every open dialog listening on `document`
 * means one Escape closes all of them at once.
 */
...
/** The current depth. Never read this to make a rendering decision — it is
 *  module state and does not notify React. */
...
export function useDialogFocus(
  ...
  // honest. Most callers pass an inline arrow, so an `onClose` dependency would
  // tear down and re-register on every render of the owning component, and each
  // re-registration would push that dialog back to the TOP of the stack. A
  // sheet that merely re-rendered would then steal Escape from the palette
  // summoned over it — the exact bug the stack exists to prevent, reintroduced
  ...
  // Capture the opener before descendant passive effects run. A drawer that
  // deliberately focuses its search input in a child useEffect would otherwise
  // race ahead of a parent passive effect, and the restore would land on the
  // drawer's soon-to-be-removed input instead of on the trigger.
  useLayoutEffect(() => {
    if (open) {
