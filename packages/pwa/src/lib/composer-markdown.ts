 ...
 *
 * Ported from kteam's `src/lib/composer-markdown.ts`. It is not a markdown
 * renderer. The overlay painted behind the composer textarea needs to reproduce
 * the EXACT text the reader typed, byte for byte, with colour spans over the
 * markdown syntax. Two hard rules follow:
 *
 ...
 *      normalised character would shift the overlay glyphs relative to the real
 *      (transparent) textarea text and the caret would visibly drift. The test
 *      suite asserts this property on every fixture.
 *
 *   2. BEST-EFFORT SEMANTICS. This colours syntax while the reader is
 *      MID-KEYSTROKE, so "wrong" states are the common case: an unterminated
 *      ``` fence is a code block to the end of the draft (that is what the
 *      reader is about to have), a lone `*` is plain text, a half-typed link is
 *      plain text. It never throws and never guesses beyond the current draft.
 *
 * Why not a highlighting grammar from the syntax registry the transcript
 * already bundles? It emits HTML with its own escaping, and its token
 * boundaries are not guaranteed lossless across versions — while losslessness
 * is exactly this overlay's correctness contract. An owned tokenizer is cheaper
 * than auditing that contract on every registry bump.
 */
