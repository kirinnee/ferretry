 ...
 *
 * Two things went wrong in kteam that a static render test could never have
 * caught, because both are about what happens between two awaits:
 *
 ...
 *      reader lets go and the control calls `stop()`. The capture session
 *      correctly shared one flush — but each CALLER continued past it, opened
...
import { captureErrorFrom } from './capture-error.ts';
import { SttRequestError } from './daemon-engine.ts';
import { durationSeconds, MIN_UTTERANCE_SECONDS } from './pcm.ts';

/** What the control shows the reader, and what the shortcut gesture reads. */
export type DictationPhase = 'idle' | 'requesting' | 'recording' | 'transcribing' | 'error';

/**
 * The slice of a capture session this lifecycle needs. Narrow on purpose: a
...
export type FinishPhase = Extract<DictationPhase, 'transcribing' | 'idle' | 'error'>;
...
const failureCode = (failure: unknown): string => (failure instanceof SttRequestError ? failure.code : 'unknown');

/** The transcribe/refine/commit half, once the samples are known to be usable. */
const transcribeAndCommit = async (token: number, samples: Float32Array, deps: FinishDeps): Promise<void> => {
  ...
  try {
    ...
    deps.setError({ code: failureCode(failure), message: failureMessage(failure) });
    deps.setPhase('error');
  } finally {
...
export async function finishUtterance(token: number, deps: FinishDeps): Promise<void> {
  ...
  try {
    ...
    await transcribeAndCommit(token, samples, deps);
  } finally {
