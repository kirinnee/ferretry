...
export const insertTranscript = (
  ...
  if (spoken.length === 0) return { text: base, caret: clamp(selectionEnd, 0, base.length) };
  ...
  return {
    ...
    caret: before.length + (needsLeadingSpace ? 1 : 0) + spoken.length,
  };
};
...
export const readSelection = (element: SelectionLike | null | undefined, draft: string): readonly [number, number] => {
