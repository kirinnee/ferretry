...
/**
 * Operator-owned per-model prices for this daemon, validated by the shape the browser also edits.
 *
 * THE SCHEMA IS NOT SPELLED HERE. A price is read by the daemon and written by the pricing surface,
 * and a daemon-only validator would let the two disagree about what a valid entry is — each passing
 * its own fixture, and differing only in front of the person whose numbers were refused. The empty
 * default stays: a daemon must report a missing price as unpriced, never guess from a public table
 * or turn absence into a zero-cost result.
 */
export { AnalyticsPricingCatalogSchema, ConfiguredAnalyticsPricingSourcesSchema };
...
export const DaemonConfigDocumentSchema = z
  .object({
    ...
    /**
     * The provider feeds this daemon may refresh those prices from.
     *
     * THE ADDRESSES LIVE HERE, not in the request that triggers a sync. A preview names one of these
     * by its `id`, so a browser — or anything holding a browser's credential — can ask this daemon to
     * fetch, and cannot choose where from. The empty default means a daemon syncs from nothing until
     * its operator says otherwise, which is the same posture as the empty catalog above.
     */
